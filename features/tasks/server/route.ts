...
const app = new Hono()
   ...
   .post("/", sessionMiddleware, zValidator("json", createTaskSchema), async (c) => {
      ...
      const newPosition =
         highestPosiotionTask.documents.length > 0
            ? highestPosiotionTask.documents[0].position + 1000
            : 1000;
   ...
   .patch(
      ...
      async (c) => {
         const user = c.get("user");
         const databases = c.get("databases");
         const { name, status, dueDate, description, projectId, assigneeId } =
            c.req.valid("json");
         const { taskId } = c.req.param();
   ...
   .post(
      ...
      async (c) => {
         ...
         if (!workspaceId) return c.json({ error: "wokspaceId was not found" });
